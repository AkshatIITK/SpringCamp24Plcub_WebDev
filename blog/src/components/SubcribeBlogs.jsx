...
export default function SubscribedBlog({user}) {
  // console.log(user)
    ...
    useEffect(() => {
      async function fetchData() {
          try {
              const res = await fetch("http://localhost:8080/SubcribedBlogs",{
                method: "POST",
                headers: {
        ...
        <div className="blogs-wrapper">
              <Box>
                  ...
                  {Actions.map((Action, index) => (
                        <OutlinedCard key={index} blog={Action} user={user} sub={true}></OutlinedCard>
                    ))}
                  <ScrollTop {...Actions}>
